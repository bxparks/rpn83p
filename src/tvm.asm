...
RclTvmSolverCount:
    ld a, (tvmSolverCount)
    call convertU8ToOP1FP1 ; OP1=float(A)
    ret
...
compoundingFactors:
    ...
    call op1ToOp2FP1 ; OP2=(1+ip)[(1+i)^N-1]/i
    ...
    call exchangeFPSFPSFP1 ; FPS=[N*ln(1+i),i]
    call ExpMinusOne ; OP1=exp(N*ln(1+i))-1
    call exchangeFPSOP1FP1 ; FPS=[N*ln(1+i),exp(N*ln(1+i))-1]; OP1=i
    ...
    call exchangeFPSOP1FP1 ; FPS=[CF3]; OP1=N*ln(1+i)
...
tvmCheckUpdateSumSign:
    call signOfOp1FP1 ; A=signbit(OP1)
    add a, c
...
inverseCompoundingFactor:
    ...
    call exchangeFPSOP1FP1 ; FPS=[N,N*i]; OP1=i
    call LnOnePlus ; OP1=ln(1+i)
    call exchangeFPSFPSFP1 ; FPS=[N*i,N]
    bcall(_PopRealO2) ; FPS=[Ni]; OP2=N
    bcall(_FPMult) ; OP1=N*ln(1+i)
    call ExpMinusOne ; OP1=exp(N*ln(1+i))-1
    call op1ToOp2FP1 ; OP2=exp(N*ln(1+i))-1
...
nominalPMT:
    ...
    call op1ToOp2FP1 ; OP2=i
    call RclTvmN ; OP1=N
    call inverseCompoundingFactor ; OP1=ICFN(N,i)
    call op1ToOp2FP1 ; OP2=ICFN(N,i)
    call RclTvmFV ; OP1=FV
    bcall(_FPMult) ; OP1=FV*ICFN(N,i)
    call exchangeFPSOP1FP1 ; FPS=[FV*ICFN()]; OP1=i
    ...
    call exchangeFPSOP1FP1 ; FPS=[FV*ICFN(N,i),PV*ICFN(-N,i)]; OP1=i
    ; Calculate (1+ip)PMT*N
    call beginEndFactor ; OP1=(1+ip)
    call op1ToOp2FP1 ; OP2=(1+ip)
...
calculateNextSecantInterest:
    ...
    call exchangeFPSFPSFP1 ; FPS=[npmt1,npmt0,i0*npmt1]
    bcall(_PopRealO2) ; FPS=[npmt1,npmt0]; OP2=i0*npmt1
    bcall(_InvSub) ; OP1=i0*npmt1-i1*npmt0
    call exchangeFPSOP1FP1 ; FPS=[npmt1,i0*npmt1-i1*npmt0]; OP1=npmt0
    call op1ToOp2FP1 ; OP2=npmt0
    call exchangeFPSFPSFP1 ; FPS=[i0*npmt1-i1*npmt0,npmt1]; OP2=npmt0
    bcall(_PopRealO1) ; FPS=[i0*npmt1-i1*npmt0]; OP1=npmt1; OP2=npmt0
    bcall(_FPSub)  ; OP1=npmt1-npmt0
    call op1ToOp2FP1 ; OP2=npmt1-npmt0
...
TvmCalculateN:
    ...
    call exchangeFPSOP1FP1 ; FPS=[ln(1+N0)]; OP1=i
    call LnOnePlus ; OP1=ln(1+i); throws exception if 1+i<=0
    call op1ToOp2FP1 ; OP2=ln(1+i)
...
TvmCalculatePMT:
    ...
    call op1ToOp2FP1 ; OP2=CF1
    call RclTvmPV ; OP1=PV
    bcall(_FPMult) ; OP1=PV*CF1
    call op1ToOp2FP1 ; OP2=PV*CF1
    call RclTvmFV ; OP1=FV
...
TvmCalculateFV:
    ...
    call RclTvmPMT ; OP1=PMT
    bcall(_FPMult) ; OP1=PMT*CF3
    call exchangeFPSOP1FP1 ; FPS=[PMT*CF3]; OP1=CF1
    call op1ToOp2FP1 ; OP2=CF1
    call RclTvmPV ; OP1=PV
    bcall(_FPMult) ; OP1=PV*CF1
